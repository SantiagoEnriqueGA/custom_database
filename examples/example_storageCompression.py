...
def main():
    ...
    # db.create_table_from_csv("example_datasets/reviews.csv", table_name="tbl", headers=True, parrallel=True, progress=True)
    db.create_table_from_csv("example_datasets/pima-indians-diabetes.csv", "tbl", headers=True)
    ...
    loaded_db = Storage.load("example_storage/example_database.segadb", user="admin", password="password123")
    loaded_db_compressed = Storage.load("example_storage/example_database_compressed.segadb", user="admin", password="password123", compression=True)
